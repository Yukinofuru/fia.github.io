...
[uiSetMenu]
;背景画像の表示
[bg2 storage="bg_room/room_d.png" time=1500 wait=false left=180 top=80 width=1440 height=900]
...
; 動作確認
[chara_show name="setsu" top=160 width=616 height=924 eye="eye4" mouth="mouth4" hat="none"]
#setsu
「何か適当にしゃべって欲しいとのことだけど、どうして私たちなのかしら」[p]
[chara_show name="akehiru" top=40 width=714 height=1125 eye="eye5" mouth="mouth5" hat="none"]
#akehiru
「全くですよ。僕だって暇じゃないのに」[p]
#setsu
「雨中 若菜から伝言よ。『明昼は糸目だから差分が描きやすかったので、色々と表情を実装しました。[l][r]
試してもいい？』だそうよ」[p]
[akehiruNormal]
#akehiru
「はぁ、どうして”お気に入り”の方を先に実装しないんですか……」[p]
[setsuNormal]
#setsu
「さあ、納得ができなかったのではないかしら？」[p]
[akehiruSad]
#akehiru
「あと、どうして僕の帽子作画は諦めたんですか……」[p]
[setsuSmile]
#setsu
「帽子を考えないで作画した結果らしいわ」[p]
[setsuSadHat]
#setsu
「私の方だって、少し帽子がズレていて、髪の毛の差分として作るか迷ってるそうよ」[p]
[setsuSmile]
[akehiruJito]
#akehiru
「作者はもっと働くべきですね。このゲーム、本当に完成させる気あるんですか？」[p]
[itoNormal]
[chara_show name="ito" top=160 width=616 height=924]
#ito
「二人とも、見回りをサボってここで何をしてる」[p]
[akehiruSurprise]
[setsuSurprise]
[chara_hide name="ito"]
#setsu
「メタ話をこれ以上している暇は無いみたい」[p]
#akehiru
「なら、ここらへんで終了しましょうか」[p]
[setsuSmile]
[akehiruSmile]
#setsu
「それでは皆さん、引き続きデモ版を楽しんでください」[p]
...
[if exp="sf.spi >50 && tf.clickCount < 6"]
...
*clack
[cm]
[if exp="tf.clickCount == 0"]
[eval exp="tf.clickCount= tf.clickCount+1"]
[jump target="*choose1"]
[endif]

[if exp="tf.clickCount == 1"]
[eval exp="tf.clickCount= tf.clickCount+1"]
[jump target="*choose1"]
[endif]

[if exp="tf.clickCount == 2"]
...
[if exp="tf.clickCount == 3"]
[eval exp="tf.clickCount= tf.clickCount+1"]
[jump target="*choose1"]
[endif]

[if exp="tf.clickCount == 4"]
[freeimage layer="2"]
...
[bg2 storage="room_c.png" time=1500 wait=true left=180 top=80 width=1440 height=900]
#kiichi
「……俺に託すなよ……畜生……」[p]
...
[eval exp="sf.currentPlaceName = '07/07/2345 ？？？'"]
[uiSetPlace]
...
[bg2 storage="room_b.png" time=1500 wait=true left=180 top=80 width=1440 height=900]
...
[bg2 storage="bg_room/room_b.png" time=1500 wait=true left=180 top=80 width=1440 height=900]
